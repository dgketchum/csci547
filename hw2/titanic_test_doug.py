...
def make_data_matrix(data):
    ...
    X = np.column_stack((np.ones(len(y)),Pclass_1hot,data['Age'],data['SibSp'],data['Parch'],data['Fare'],Embarked_1hot))
...
def _J(w):
    w = w.reshape((n,N))
    return -np.column_stack([np.sum([(T - sigmoid(X,w))[i,k] * X[i] for i in range(m)],axis=0) for k in range(N)])#.ravel()
